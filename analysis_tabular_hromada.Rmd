...
##############################################################################################################
  ...
  source("src/format_dataset_hromada.R")  # <- additional indicators and grouping variables are added here 
...
```{r setup2, include=FALSE}
 ...
 data.list$main <- data.list$main %>% 
    mutate(strata = a7_current_hromada) 
  hromada <- data.list$main %>%
    select(a7_current_hromada,uuid) %>%
    rename(strata = a7_current_hromada)
  strata <- hromada %>% select(strata) %>% distinct()
    ...
    for (disagg.var in entry$disaggregate.variables) {
      ...
      if (is.na(entry$join)){
        ...
        if (entry$admin == "strata"){
          recode$col <- "hromada_name"
          res$hromada_name <- res$strata
          res <- res %>% matchmaker::match_df(dictionary = recode, from = "from",               
                          to = "to",                   
                          by = "col") %>%
          relocate(hromada_name, .after = strata)
          res <- res %>%
            rename("hromada" = "strata") %>%
            rename("strata" = "hromada_name") %>%
            mutate(raion = substr(hromada,1,6), .before = hromada) %>%
              mutate(oblast = substr(raion,1,4), .before = raion)
        }
