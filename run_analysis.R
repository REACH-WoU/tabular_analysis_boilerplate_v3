...
strings <- c(
  ...
  dataset.name.short = paste0("JMMI UKR ", JMMI_variable, "Region"),   # provide a short name for filenames of output documents (e.g. "POL_PDM")
  ...
  filename.tool = paste0("resources/", JMMI_variable, "/UKR2203_JMMI_Questionnaire_Customers_R15_07JUN2023_SB.xlsx"),      # filename of your kobo tool
  filename.daf.tabular = paste0("resources/", JMMI_variable, "/DAF_region.xlsx")     # filename of your DAF
)
...
rmarkdown::render('analysis_tabular_region.Rmd',
                  output_file = paste0("output/", JMMI_variable, "/", strings['dataset.name.short'], "_Tabular_Analysis_Region", strings['out_date'],".html"))
