  ...
  for (r in 1:nrow(daf.section)){
    ...
    for (disagg.var in entry$disaggregate.variables) {
      ...
      if (is.na(entry$join)){
        ...
        if (is.na(disagg.var)){         # Checking if there are disagregation variables and setting up variables to join by
          joining_vars <- "strata"
        }else{
          joining_vars <- c("strata",disagg.var)
        }
        # Setting column names as "<function>_<question_name>"
          colnames(res) <- c(joining_vars,paste(colnames(res)[length(joining_vars)+1:length(res)],entry$variable,sep="_"))
        # Joining the res table to the existing joint table or creating a new one 
        if (jt.count[entry$join,"Freq"] == 0){
          ...
          # Write the joint table to the file if it is full
