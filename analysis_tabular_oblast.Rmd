...
JMMI_variable <- c("Customers")
  ...
  source("src/init.R")
  sampling <- read_excel(paste0("resources/", JMMI_variable, "/sample.xlsx")) ##Change folder to Retailers or Customers
...
##############################################################################################################
  ...
  source("src/format_dataset_oblast.R")  # <- additional indicators and grouping variables are added here 
...
```{r setup2, include=FALSE}
 ...
 data.list$main <- data.list$main %>% 
    mutate(strata = a5_current_oblast) 
  oblast <- data.list$main %>%
    select(a5_current_oblast,uuid) %>%
    rename(strata = a5_current_oblast)
  strata <- oblast %>% select(strata) %>% distinct()

#  data.list$child_breast_feeding <- data.list$child_breast_feeding %>% 
#  left_join(oblast, by="uuid")
#
#  data.list$healthcare <- data.list$healthcare %>% 
#    left_join(oblast, by="uuid")
#
#  data.list$mental_health <- data.list$mental_health %>% 
#    left_join(oblast, by="uuid") 
#
#  data.list$hh_members <- data.list$hh_members %>% 
#    left_join(oblast, by="uuid") 
  ## SRVYR DESIGNS
  ##############################################################################
      ...
      if (is.na(entry$join)){
        # add p-codes
          if (entry$admin == "strata"){
            recode$col <- "oblast_name"
            res$oblast_name <- res$strata
            res <- res %>% matchmaker::match_df(dictionary = recode, from = "from",               
                            to = "to",                   
                            by = "col") %>%
            relocate(oblast_name, .after = strata)
            res <- res %>%
              rename("oblast" = "strata") %>%
              rename("strata" = "oblast_name")
            }
