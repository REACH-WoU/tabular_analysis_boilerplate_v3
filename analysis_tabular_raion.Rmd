...
##############################################################################################################
  ...
  source("src/format_dataset_raion.R")  # <- additional indicators and grouping variables are added here 
...
```{r setup2, include=FALSE}
 ...
 data.list$main <- data.list$main %>% 
    mutate(strata = a6_current_raion) 
  raion <- data.list$main %>%
    select(a6_current_raion,uuid) %>%
    rename(strata = a6_current_raion)
  strata <- raion %>% select(strata) %>% distinct()
    ...
    for (disagg.var in entry$disaggregate.variables) {
      ...
      if (is.na(entry$join)){
        ...
        if (entry$admin == "strata"){
          recode$col <- "raion_name"
          res$raion_name <- res$strata
          res <- res %>% matchmaker::match_df(dictionary = recode, from = "from",               
                          to = "to",                   
                          by = "col") %>%
          relocate(raion_name, .after = strata)
          res <- res %>%
            rename("raion" = "strata") %>%
            rename("strata" = "raion_name") %>%
